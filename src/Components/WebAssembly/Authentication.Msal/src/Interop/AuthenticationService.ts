...
class MsalAuthorizeService implements AuthorizeService {
    ...
    async completeSignOut(url: string) {
        ...
        const logoutState = this.retrieveState(updatedUrl.href, /*isLogout*/ true);
    ...
    retrieveState<T>(url: string, isLogout: boolean = false): T | undefined {
        let state = !isLogout && url;
        if (isLogout && url) {
            const parsedUrl = new URL(url);
            const fromHash = parsedUrl.hash && parsedUrl.hash.length > 0 && new URLSearchParams(parsedUrl.hash.substring(1));
            state = fromHash && fromHash.getAll('state')[0];
            if (state && state.length > 1) {
                return undefined;
            } else if (!state || state.length == 0) {
                state = parsedUrl.searchParams && parsedUrl.searchParams.getAll('state')[0];
                if (!state || state.length !== 1) {
                    return undefined;
                }
            }
